...
export const searchCoursesByCourseName = async (req, res) => {
    ...
    try {
        const courses = await Course.find({ courseName: { $regex: courseName, $options: "i" } });
        if (courses.length === 0) {
            return res.status(404).json({ message: "No courses found" });
        }
        res.status(200).json(courses);
...
export const searchCoursesByCourseCode = async (req, res) => {
    ...
    try {
        const courses = await Course.find({ courseCode: { $regex: courseCode, $options: "i" } });
        if (courses.length === 0) {
            return res.status(404).json({ message: "No courses found" });
        }
        res.status(200).json(courses);
...
export const searchCoursesByDepartmentNameRegex = async (req, res) => {
    ...
    try {
        const department = await Department.findOne({ departmentName: { $regex: departmentName, $options: "i" } });
        if (!department) {
            return res.status(404).json({ message: "Department not found" });
        }
        const courses = await Course.find({ departmentId: department._id });
